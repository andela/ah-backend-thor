...
class TestProfile(APITestCase):
    def setUp(self):
        ...
        self.response = self.client.post(self.register_url, self.user, format="json")
        User.objects.filter(email="david@gmail.com").update(is_verified=True)
        self.response2 = self.client.post(self.login_url, self.user, format="json")
        self.token_user = self.response2.data['user_token']
        self.headers = {'HTTP_AUTHORIZATION': "Token " + f'{self.token_user}'}

        follow_response = self.client.post(self.register_url, self.user2, format="json")
        User.objects.filter(email="jake@gmail.com").update(is_verified=True)
        follow_response2 = self.client.post(self.login_url, self.user2, format="json")
        self.token_user2 = follow_response2.data['user_token']
        self.headers2 = {'HTTP_AUTHORIZATION': "Token " + f'{self.token_user2}'}
    ...
    def test_user_profile(self):
        ...
        res = self.client.put(self.user_url, user, **self.headers, format="json")
        self.assertEqual(res.status_code, 200)
       
        res = self.client.get(self.user_profile_url, **self.headers, format="json")
        self.assertEqual(res.status_code, 200)

    def test_retrive_userprofile_detail(self):
        res = self.client.get(self.user_profile_url2, **self.headers, format="json")
        self.assertEqual(res.status_code, 200)
    ...
    def test_user_following_other_users(self):
        response3 = self.client.post(self.follow_user, **self.headers, format ='json')
        self.assertEqual('David', response3.data['following_user'])

    def test_user_following_non_existent_user(self):
        response3 = self.client.post(self.follow_non_existent_user, **self.headers, format ='json')
    ...
    def test_list_of_authors_user_followers(self):
        ...
        response3 = self.client.get('/api/profiles/jake/followers', format ='json')
    ...
    def test_list_of_authors_user_is_following(self):
        self.test_user_following_other_users()
        response3 = self.client.get('/api/profiles/David/following', format ='json')
        self.assertIn('David', response3.data['results'][0]['following_username'])

    def test_list_of_authors_user_is_following_non_existent(self):
        response3 = self.client.get('/api/profiles/jake/following', format ='json')
        print(response3.data)
        self.assertEqual([], response3.data['results'])

    def test_user_unfollow_other_users(self):
        self.test_user_following_other_users()
        response3 = self.client.delete('/api/profiles/jake/unfollow', **self.headers, format ='json')
